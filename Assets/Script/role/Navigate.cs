...
namespace com.DungeonPad
{
    public class Navigate : MonoBehaviour
    {
        ...
        public static HashSet<int> CrystalPos = new HashSet<int>(), CrystalSidePos = new HashSet<int>();
        public bool Boss;
                    ...
                    if (canGoRoom[i, j] != -1)
                    {
                        for (k = 0; k < GameManager.mazeCreater.objectCountRowNum; k++)
                        {
                            ...
                            for (l = 0; l < GameManager.mazeCreater.objectCountColNum; l++)
                            {
                                ...
                                if (!(temp == "wall" || temp == "nm" || temp.Contains("Door") || temp.Contains("Hole") || temp.Contains("hole")))
                                {
            ...
            if (!findingRoad)
            {
                ...
                for (i = 0; i < MazeCreater.totalRow; i++)
                {
                    for (j = 0; j < MazeCreater.totalCol; j++)
                    {
                        ...
                        if (!canGo.ContainsValue(pos) || CrystalPos.Contains(pos) || (Boss && CrystalSidePos.Contains(pos)))
                        {
                            close.Add(pos);
                        }
                    }

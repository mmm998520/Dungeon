...
{
    public class SkeletonKnight : MonsterManager
    {
        ...
        void Start()
        {
            range = new Transform[30];
            for (int j = 0; j < 30; j++)
            {
                range[j] = GameManager.Floors.GetChild(rangeRow[j] * MazeGen.col + rangeCol[j]);
            }
            monsterStart();
            cd = 2;
            moveSpeed = 1;
        ...
        WaitForSeconds AttackWait = new WaitForSeconds(0.5f);
